...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    version="2.0"
    xmlns:myData="http://www.bartneck.de"
    xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl">
    <xsl:template match="/">
        <html>
            <head>
                <title><xsl:for-each select="program"><xsl:value-of select="title"/></xsl:for-each></title>
                
            </head>
            <body>
                <h1><xsl:for-each select="program"><xsl:value-of select="title"/></xsl:for-each></h1>
    ...
    <xsl:template match="instruction">
        <li><xsl:value-of select="lengthAsDistance"/></li> 
    ...
    <xsl:template name="dateTransformation">
        test
        <xsl:for-each select="program">
